...
void PmergeMe::display(std::vector<pairs> out) {
	std::cout << "After:  ";
...
size_t PmergeMe::BS_vec(std::vector<pairs> res, size_t right, int value) {
	size_t left = 0, mid;
...
size_t PmergeMe::BS_list(std::list<graph> res, size_t right, int value) {
    size_t left = 0, mid;
...
std::list<graph> PmergeMe::list_sort(std::list<graph> arr)
{
    ...
    else if (arr.size() == 2)
    {
        ...
        std::list<graph>::iterator it = arr.begin();
    ...
    while (!small.empty())
    {
        ...
        for (size_t i = 0; i < groupsize; ++i)
        {
            ...
            const size_t BS_point   = groupsize - i - 1;
            const size_t right_side = 1 + group_passed*2 + BS_point + i;
            ...
            size_t insert_index = BS_list(res, right_side, val.num);
